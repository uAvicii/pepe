...
const { loadings, onDel } = useDelOrder(() => {
  router.push('/consult')
})
...
<template>
  <div class="consult-detail-page" v-if="detail">
    ...
    <div class="detail-time" v-if="detail.status === OrderType.ConsultPay">
      请在 <van-count-down :time="detail.countdown! * 1000" /> 内完成支付，超时订单将取消
    </div>
    <!-- 待支付 -->
    <div class="detail-action van-hairline--top" v-if="detail.status === OrderType.ConsultPay">
      ...
      <van-button type="default" round :loading="loading" @click="onCancel(detail)">
        取消问诊
      </van-button>
  ...
  <div class="consult-detail-page" v-else>
    <cp-nav-bar title="问诊详情" />
    <van-skeleton title :row="6" style="margin-top: 30px" />
    <van-skeleton title :row="6" style="margin-top: 30px" />
  </div>
</template>
